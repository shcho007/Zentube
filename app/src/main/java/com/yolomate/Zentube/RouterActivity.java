...
import com.yolomate.Zentube.player.helper.PlayerHelper;
import com.yolomate.Zentube.playlist.ChannelPlayQueue;
import com.yolomate.Zentube.playlist.PlayQueue;
import com.yolomate.Zentube.playlist.PlaylistPlayQueue;
...
import com.yolomate.Zentube.playlist.SinglePlayQueue;
import com.yolomate.Zentube.report.UserAction;
import com.yolomate.Zentube.util.PermissionHelper;
...
public class RouterActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (TextUtils.isEmpty(currentUrl)) {
            ...
            if (TextUtils.isEmpty(currentUrl)) {
                Toast.makeText(this, com.yolomate.Zentube.R.string.invalid_url_toast, Toast.LENGTH_LONG).show();
                finish();
            }
        }

        setTheme(ThemeHelper.isLightThemeSelected(this)
                ? com.yolomate.Zentube.R.style.RouterActivityThemeLight
                : com.yolomate.Zentube.R.style.RouterActivityThemeDark);
    }
    ...
    private void handleError(Throwable error) {
        ...
        if (error instanceof ExtractionException) {
            Toast.makeText(this, com.yolomate.Zentube.R.string.url_not_supported_toast, Toast.LENGTH_LONG).show();
        } else {
    ...
    protected void onSuccess() {
        ...
        boolean isExtVideoEnabled = preferences.getBoolean(getString(com.yolomate.Zentube.R.string.use_external_video_player_key), false);
        boolean isExtAudioEnabled = preferences.getBoolean(getString(com.yolomate.Zentube.R.string.use_external_audio_player_key), false);

        if ((isExtAudioEnabled || isExtVideoEnabled) && currentLinkType != LinkType.STREAM) {
            Toast.makeText(this, com.yolomate.Zentube.R.string.external_player_unsupported_link_type, Toast.LENGTH_LONG).show();
            finish();
            return;
        }
        ...
        if (currentService == ServiceList.SoundCloud) {
            handleChoice(getString(com.yolomate.Zentube.R.string.background_player_key));
            return;
        }

        final String playerChoiceKey = preferences.getString(
                getString(com.yolomate.Zentube.R.string.preferred_open_action_key),
                getString(com.yolomate.Zentube.R.string.preferred_open_action_default));
        final String alwaysAskKey = getString(com.yolomate.Zentube.R.string.always_ask_open_action_key);
    ...
    private void showDialog() {
        ...
        final ContextThemeWrapper themeWrapper = new ContextThemeWrapper(this,
                ThemeHelper.isLightThemeSelected(this) ? com.yolomate.Zentube.R.style.LightTheme : com.yolomate.Zentube.R.style.DarkTheme);

        LayoutInflater inflater = LayoutInflater.from(themeWrapper);
        final LinearLayout rootLayout = (LinearLayout) inflater.inflate(com.yolomate.Zentube.R.layout.preferred_player_dialog_view, null, false);
        final RadioGroup radioGroup = rootLayout.findViewById(android.R.id.list);

        final AdapterChoiceItem[] choices = {
                new AdapterChoiceItem(getString(com.yolomate.Zentube.R.string.show_info_key), getString(com.yolomate.Zentube.R.string.show_info),
                        ThemeHelper.resolveResourceIdFromAttr(themeWrapper, com.yolomate.Zentube.R.attr.info)),
                new AdapterChoiceItem(getString(com.yolomate.Zentube.R.string.video_player_key), getString(com.yolomate.Zentube.R.string.video_player),
                        ThemeHelper.resolveResourceIdFromAttr(themeWrapper, com.yolomate.Zentube.R.attr.play)),
                new AdapterChoiceItem(getString(com.yolomate.Zentube.R.string.background_player_key), getString(com.yolomate.Zentube.R.string.background_player),
                        ThemeHelper.resolveResourceIdFromAttr(themeWrapper, com.yolomate.Zentube.R.attr.audio)),
                new AdapterChoiceItem(getString(com.yolomate.Zentube.R.string.popup_player_key), getString(com.yolomate.Zentube.R.string.popup_player),
                        ThemeHelper.resolveResourceIdFromAttr(themeWrapper, com.yolomate.Zentube.R.attr.popup))
        };
        ...
        final DialogInterface.OnClickListener dialogButtonsClickListener = (dialog, which) -> {
            ...
            if (which == DialogInterface.BUTTON_POSITIVE) {
                preferences.edit().putString(getString(com.yolomate.Zentube.R.string.preferred_open_action_key), choice.key).apply();
            }
        };

        final AlertDialog alertDialog = new AlertDialog.Builder(themeWrapper)
                .setTitle(com.yolomate.Zentube.R.string.preferred_player_share_menu_title)
                .setView(radioGroup)
                .setCancelable(true)
                .setNegativeButton(com.yolomate.Zentube.R.string.just_once, dialogButtonsClickListener)
                .setPositiveButton(com.yolomate.Zentube.R.string.always, dialogButtonsClickListener)
        ...
        for (AdapterChoiceItem item : choices) {
            final RadioButton radioButton = (RadioButton) inflater.inflate(com.yolomate.Zentube.R.layout.list_radio_icon_item, null);
        ...
        if (selectedRadioPosition == -1) {
            final String lastSelectedPlayer = preferences.getString(getString(com.yolomate.Zentube.R.string.preferred_open_action_last_selected_key), null);
    ...
    private void handleChoice(final String playerChoiceKey) {
        if (Arrays.asList(getResources()
                .getStringArray(com.yolomate.Zentube.R.array.preferred_open_action_values_list))
                .contains(playerChoiceKey)) {
            PreferenceManager.getDefaultSharedPreferences(this).edit()
                    .putString(getString(com.yolomate.Zentube.R.string.preferred_open_action_last_selected_key),
                            playerChoiceKey).apply();
        }

        if (playerChoiceKey.equals(getString(com.yolomate.Zentube.R.string.popup_player_key))
        ...
        if(playerChoiceKey.equals(getString(com.yolomate.Zentube.R.string.show_info_key))) {
            disposables.add(Observable
    ...
    public static class FetcherService extends IntentService {
        ...
        public Consumer<Info> getResultHandler(Choice choice) {
            return info -> {
                final String videoPlayerKey = getString(com.yolomate.Zentube.R.string.video_player_key);
                final String backgroundPlayerKey = getString(com.yolomate.Zentube.R.string.background_player_key);
                final String popupPlayerKey = getString(com.yolomate.Zentube.R.string.popup_player_key);
        ...
        private NotificationCompat.Builder createNotification() {
            return new NotificationCompat.Builder(this, getString(com.yolomate.Zentube.R.string.notification_channel_id))
                    .setOngoing(true)
                    .setSmallIcon(com.yolomate.Zentube.R.drawable.ic_newpipe_triangle_white)
                    .setVisibility(NotificationCompat.VISIBILITY_PUBLIC)
                    .setContentTitle(getString(com.yolomate.Zentube.R.string.preferred_player_fetcher_notification_title))
                    .setContentText(getString(com.yolomate.Zentube.R.string.preferred_player_fetcher_notification_message));
        }
    }
